...
const ContextMenu = ({
  ...
  /** 
   * Remove contextmenu 
   * Works for both desktop (clicks) and mobile (touch).
   */
  ...
  /** 
   * Arrow event for keyboard navigation
   */
  ...
  useEffect(() => {
    ...
    if (active) {
      ...
      node && node.addEventListener('touchstart', onRemove); // Для мобильных устройств
      node && node.addEventListener('keyup', onKeyPress);
      document.addEventListener('mousedown', onRemove);
      document.addEventListener('touchstart', onRemove); // Для мобильных устройств
      ...
      node && node.removeEventListener('touchstart', onRemove); // Для мобильных устройств
      node && node.removeEventListener('keyup', onKeyPress);
      document.removeEventListener('mousedown', onRemove);
      document.removeEventListener('touchstart', onRemove); // Для мобильных устройств
      document.removeEventListener('keyup', onKeyPress);
    }

    return () => {
      node && node.removeEventListener('mousedown', onContextmMenuRemove);
      node && node.removeEventListener('touchstart', onRemove);
      node && node.removeEventListener('keyup', onKeyPress);
      document.removeEventListener('keyup', onKeyPress);
    };
  }, [
